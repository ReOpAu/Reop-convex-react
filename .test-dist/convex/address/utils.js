...
function cleanSuburbString(str) {
    return str
...
export function extractSuburbFromPlacesSuggestion(suggestion) {
    if (suggestion.resultType === "suburb" &&
...
function classifyResultType(types, description) {
...
function calculateConfidence(prediction, detectedIntent, resultType, searchQuery, position) {
    ...
    let confidence = position !== undefined
    ...
    if (searchQuery && prediction.description) {
        const similarity = calculateStringSimilarity(searchQuery.toLowerCase().trim(), prediction.description.toLowerCase());
        confidence += similarity * 0.2;
    }
    // 4. AUSTRALIAN GEOLOCATION RELEVANCE (+15%)
    // Enhanced detection of Australian address patterns
    const australianRelevance = calculateAustralianRelevance(prediction.description);
    ...
    const establishmentScore = calculateEstablishmentRelevance(prediction.types, detectedIntent);
    ...
    if (prediction.matched_substrings &&
        ...
        const totalMatched = prediction.matched_substrings.reduce((sum, match) => sum + match.length, 0);
...
function calculateStringSimilarity(query, result) {
...
function calculateAustralianRelevance(description) {
    let relevance = 0;
    ...
    if (/(Street|Road|Avenue|Lane|Drive|Court|Place|Terrace|Crescent|Circuit)\b/i.test(description)) {
        relevance += 0.1;
    }
    return Math.min(relevance, 1.0);
}
function calculateGoogleQualityScore(prediction) {
    let quality = 0;
...
function calculateEstablishmentRelevance(types, intent) {
...
function calculateAddressCompleteness(description) {
    if (!description)
        return 0;
    let completeness = 0;
...
function doesResultMatchIntent(resultType, intent) {
    switch (intent) {
        case "suburb":
...
function shouldIncludeResult(prediction, intent, strictness) {
    ...
    const hasUnwantedType = types.some((type) => unwantedTypes.includes(type));
    ...
    if (strictness === "high") {
        if (intent === "suburb") {
            const isGenuineSuburb = types.some((type) => ["locality", "sublocality", "administrative_area_level_2"].includes(type));
            return isGenuineSuburb && !hasUnwantedType;
        }
    }
    return !hasUnwantedType;
}
export async function getPlacesApiSuggestions(query, actualIntent, maxResults, apiKey, location, radius, sessionToken, isAutocomplete) {
    const getStrictness = (intent) => {
        switch (intent) {
            case "suburb":
                return "high";
            case "street":
    ...
    const convertMatches = (matches) => (matches || []).map((m) => ({
    ...
    const predictions = (data.suggestions || [])
        ...
        return {
            ...
            structured_formatting: {
                ...
                main_text_matched_substrings: convertMatches(pp.structuredFormat?.mainText?.matches),
            },
        };
    });
    for (let i = 0; i < predictions.length; i++) {
        ...
        const confidence = calculateConfidence(prediction, actualIntent, resultType, query, i);
...
async function validateAddressOnly(address, apiKey) {
    const requestBody = {
        address: {
            regionCode: "AU",
    ...
    if (hasHouseNumber) {
        ...
        if (validationGranularity !== "PREMISE" &&
    ...
    const hasSignificantLocationChange = () => {
        const inputHasSuburb = inputWords.some((w) => !w.match(/^\d+$/) &&
            ![
                "st",
                "street",
                "rd",
        ...
        const outputHasSuburb = outputWords.some((w) => !w.match(/^\d+$/) &&
            ![
                "st",
                "street",
                "rd",
...
export async function validateThenEnrichAddress(query, maxResults, apiKey, location) {
    ...
    const placesResult = await getPlacesApiSuggestions(validationResult.formattedAddress || query, "address", maxResults, apiKey, location);
