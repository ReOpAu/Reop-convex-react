...
import { createListingValidator, updateListingValidator, } from "./schemas/listings/validator";
import { getCurrentUser, isAdminIdentity, requireAdmin, requireCurrentUser, requireIdentity, requireListingOwnerOrAdmin, } from "./utils/auth";
...
function applyListingFilters(listings, { listingType, state, suburb, }) {
    let results = listings;
    if (listingType) {
        results = results.filter((listing) => listing.listingType === listingType);
    }
    if (state) {
        results = results.filter((listing) => listing.state.toLowerCase() === state.toLowerCase());
    }
    if (suburb) {
        const normalizedSuburb = normalizeSuburbFilter(suburb);
        results = results.filter((listing) => listing.suburb.toLowerCase() === normalizedSuburb.toLowerCase());
    }
    return results;
}
...
export const listListings = query({
    ...
    handler: async (ctx, { listingType, state, suburb, page = 1, pageSize = 12 }) => {
        ...
        const filteredListings = applyListingFilters(sortListingsByNewest(allListings.filter((listing) => listing.isActive === true)), { listingType, state, suburb });
...
export const listMyListings = query({
    ...
    handler: async (ctx, { listingType, page = 1, pageSize = 24 }) => {
        ...
        const listings = await ctx.db.query("listings").collect();
        const filteredListings = sortListingsByNewest(listings.filter((listing) => listing.userId === user._id &&
...
export const listListingsAdmin = query({
    ...
    handler: async (ctx, { listingType, state, suburb, page = 1, pageSize = 20 }) => {
        await requireAdmin(ctx);
        const allListings = await ctx.db.query("listings").collect();
        const filteredListings = applyListingFilters(sortListingsByNewest(allListings), { listingType, state, suburb });
        const { items, pagination } = paginate(filteredListings, page, pageSize);
        return {
            listings: items,
            pagination,
...
export const getUserSavedListings = query({
    ...
    handler: async (ctx) => {
        ...
        const savedListings = await ctx.db
            ...
            .collect();
        const listingsWithDetails = await Promise.all(savedListings.map(async (saved) => {
...
export const getStateListingStats = query({
    ...
    handler: async (ctx, { state }) => {
        const allListings = await ctx.db.query("listings").collect();
        const stateListings = allListings.filter((listing) => listing.isActive === true &&
