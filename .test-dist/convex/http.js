...
function getAllowedOrigin(request) {
    ...
    const allowedOrigins = new Set([process.env.FRONTEND_URL].filter((value) => typeof value === "string" && value.length > 0));
...
async function requireHttpIdentity(ctx) {
    try {
...
function parseNearbyPlacesRequest(body) {
    const lat = Number(body?.lat);
    const lng = Number(body?.lng);
    const radius = body?.radius === undefined ? 2000 : Number(body.radius);
    const minRating = body?.minRating === undefined ? undefined : Number(body.minRating);
    const types = Array.isArray(body?.types)
        ? body.types.filter((type) => typeof type === "string").slice(0, 10)
        : [];
...
export const nearbyPlaces = httpAction(async (ctx, request) => {
    ...
    if (!GOOGLE_MAPS_API_KEY) {
        return jsonResponse(request, { error: "Google Places API key is not configured." }, 500);
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return jsonResponse(request, { error: "Failed to fetch nearby places." }, response.status);
        }
        const data = await response.json();
        const rawPlaces = Array.isArray(data?.places) ? data.places : [];
        let places = rawPlaces.map((place, index) => ({
            ...
            distance: calculateDistance(parsed.lat, parsed.lng, place.location.latitude, place.location.longitude),
        }));
        const minRating = parsed.minRating;
        if (minRating !== undefined) {
            places = places.filter((place) => typeof place.rating === "number" && place.rating >= minRating);
        }
        ...
        return jsonResponse(request, { error: "An internal error occurred." }, 500);
    }
});
...
function buildPreflightHandler(methods) {
