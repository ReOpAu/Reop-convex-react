...
import { clearAllListings, getAllListingsDebug, getListingForEdit, isListingSaved, listListingsAdmin, listMyListings, saveListing, } from "../convex/listings.js";
import { checkUserSubscriptionStatus, createCustomerPortalUrl, fetchUserSubscription, handleWebhookEvent, } from "../convex/subscriptions.js";
import { clearSession, getLatestUpdate, pushUpdate, registerSession, } from "../convex/cartesia/sessionState.js";
import { deleteAllListings, deleteSampleData, seedListings, } from "../convex/seedListings.js";
    ...
    query(table) {
        ...
        return {
            withIndex: (_indexName, build) => {
                ...
                build(builder);
                const filtered = () => getDocs().filter((doc) => filters.every(({ field, value }) => doc[field] === value));
                return {
                    collect: async () => filtered(),
                    first: async () => filtered()[0] ?? null,
                    unique: async () => {
                        const results = filtered();
                        assert.ok(results.length <= 1, `Expected unique result, received ${results.length}`);
...
test("listing reads stay safe before a users row exists", async () => {
    ...
    const listResult = await listMyListingsHandler({ db, auth: createAuth(identity) }, {});
    assert.deepEqual(listResult.listings, []);
    assert.equal(listResult.pagination.totalCount, 0);
    const editResult = await getListingForEditHandler({ db, auth: createAuth(identity) }, { id: listingId });
...
test("subscription reads fall back to authenticated identifiers before bootstrap", async () => {
    ...
    const subscription = await fetchHandler({ db, auth: createAuth(identity) }, {});
...
test("saved listings work for canonical and legacy user identity matches", async (t) => {
    ...
    await t.test("canonical tokenIdentifier lookup", async () => {
        ...
        await saveListingHandler({ db, auth: createAuth(identity) }, { listingId: "listings:canonical" });
        const saved = await isListingSavedHandler({ db, auth: createAuth(identity) }, { listingId: "listings:canonical" });
    ...
    await t.test("legacy subject fallback upgrades tokenIdentifier", async () => {
        ...
        await saveListingHandler({ db, auth: createAuth(identity) }, { listingId: "listings:legacy" });
        const upgradedUser = await db.get(userId);
        assert.equal(upgradedUser?.tokenIdentifier, identity.tokenIdentifier);
        const saved = await isListingSavedHandler({ db, auth: createAuth(identity) }, { listingId: "listings:legacy" });
        assert.equal(saved, true);
    });
});
    ...
    await withEnv({
        ...
        for (const [name, handler, args] of handlers) {
            await t.test(name, async () => {
                await assert.rejects(() => handler({ auth }, args), /Admin access required/);
            });
        }
    });
});
...
test("customer portal creation is still auth-gated", async () => {
    ...
    await assert.rejects(() => handler({ auth: createAuth(null) }, {}), /Not authenticated/);
});
test("Cartesia session ownership and bridge token protections hold", async () => {
    ...
    await withEnv({
        ...
        await assert.rejects(() => registerHandler({ db, auth: createAuth(intruder) }, { sessionId }), /Session is already owned by another user/);
        await assert.rejects(() => pushHandler({
            db,
        }, {
            sessionId,
        ...
        await pushHandler({ db }, {
            sessionId,
            updateType: "selection",
            data: "{\"step\":1}",
            bridgeToken: "bridge-secret",
        });
        const ownerUpdate = await latestHandler({ db, auth: createAuth(owner) }, { sessionId });
        assert.equal(ownerUpdate?.version, 1);
        assert.equal(ownerUpdate?.updateType, "selection");
        const intruderUpdate = await latestHandler({ db, auth: createAuth(intruder) }, { sessionId });
        assert.equal(intruderUpdate, null);
        await assert.rejects(() => clearHandler({ db, auth: createAuth(intruder) }, { sessionId }), /Not authorized to clear this Cartesia session/);
